...
class Futils
{
    ...
    public const CACHE_USERS = 'users';

    public const CACHE_SEARCH = 'search';
    ...
    public static array $messages = [
        ...
        'auth/internal-error' => 'An internal AuthError has occurred. reason list (  billing not enabled )',
    ...
    /**
     * Get a human-readable message from an error code.
     */
    public static function message(string $code, ?string $default = null): string
    {
        return self::$messages[$code] ?? $default ?? $code;
    }

    /**
     * Normalize any value into a deterministic cache-friendly string.
     */
    public static function normalizeCacheValue(mixed $value = null): string
    {
        try {
    ...
    /**
     * Generate a consistent cache key for Firebase Auth operations.
     */
    public static function cacheKey(?string $name = null, mixed $value = null): string
    {
        $section = Str::slug($name ?? 'mixed');
        $hash = self::normalizeCacheValue($value);

        return "fauth_{$section}_{$hash}";
    }

    /**
     * Generate cache key for a user by given uid.
     */
    public static function userCacheKey(?string $uid): string
    {
        return self::cacheKey(self::CACHE_USER, $uid);
    }

    /**
     * Get the default cache lifetime for fauth (configurable).
     */
    public static function cacheLife(): \Illuminate\Support\Carbon
    {
        return now()->addHour();
    }
     ...
     */
    public static function catchup(callable $callback, $default = null)
    {
        try {
            return $callback();
        } catch (Throwable) {
